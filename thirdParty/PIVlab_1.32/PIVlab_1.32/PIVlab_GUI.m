...
Matlab VFV Toolbox 1.0 by Nima Bigdely Shamlo
%}
...
try
    ...
    ctr=0;
    pivFiles = {'PIVlab_GUI.fig' 'dctn.m' 'idctn.m' 'inpaint_nans.m' 'piv_DCC.m' 'piv_FFTmulti.m' 'PIVlab_preproc.m' 'PIVlablogo.jpg' 'smoothn.m' 'uipickfiles.m' 'PIVlab_settings_default.mat' 'hsbmap.mat' 'rainbow.mat' 'ellipse.m' 'nanmax.m' 'nanmin.m' 'nanstd.m' 'nanmean.m' 'exportfig.m' 'fastLICFunction.m' 'icons.mat'};
...
set(gca, 'ydir', 'reverse');
text (290,500,['version: ' retr('PIVver')], 'fontsize', 7);
        ...
        if displaywhat==1 %vectors only
            ...
            if size(derived,2)>=(currentframe+1)/2 && numel(derived{displaywhat-1,(currentframe+1)/2})>0 %derived parameters requested and existant
                ...
                if get(handles.displ_colorbar,'value')==1
                    ...
                    if strmatch(posichoice,'East')==1 | strmatch(posichoice,'West')==1
                        set(coloobj,'YTickLabel',num2str(get(coloobj,'YTick')','%5.5g'))
                    end
                    if strmatch(posichoice,'North')==1 | strmatch(posichoice,'South')==1
...
switchui('multip08');
if retr('caluv')==1 && retr('calxy')==1
    set(handles.derivchoice,'String',{'Vectors [px/frame]';'Vorticity [1/frame]';'Velocity magnitude [px/frame]';'u component [px/frame]';'v component [px/frame]';'Divergence [1/frame]';'Vortex locator [1]';'Shear rate [1/frame]';'Strain rate [1/frame]';'Line integral convolution (LIC) [1]'});
    set(handles.text35,'String','u [px/frame]:')
    set(handles.text36,'String','v [px/frame]:')
else
    set(handles.derivchoice,'String',{'Vectors [m/s]';'Vorticity [1/s]';'Velocity magnitude [m/s]';'u component [m/s]';'v component [m/s]';'Divergence [1/s]';'Vortex locator [1]';'Shear rate [1/s]';'Strain rate [1/s]';'Line integral convolution (LIC) [1]'});
...
if size(filepath,1) >1
    ...
    out = PIVlab_preproc (img,roirect,clahe, clahesize,highp,highpsize,clip,clipthresh,intenscap);
...
if numel(filepath)>1
    ...
    clipthresh=str2double(get(handles.clip_thresh, 'string'));
    roirect=retr('roirect');
    A = PIVlab_preproc (A,roirect,clahe, clahesize,highp,highpsize,clip,clipthresh,intenscap);
    A(A<=80)=0;
    A(A>80)=255;
...
if ok==1
    ...
    for i=1:2:size(filepath,1)
        ...
        if isempty(cancel)==1 || cancel ~=1
            ...
            clipthresh=str2double(get(handles.clip_thresh, 'string'));
            roirect=retr('roirect');
            image1 = PIVlab_preproc (image1,roirect,clahe, clahesize,highp,highpsize,clip,clipthresh,intenscap);
            image2 = PIVlab_preproc (image2,roirect,clahe, clahesize,highp,highpsize,clip,clipthresh,intenscap);
            maskiererx=retr('maskiererx');
            maskierery=retr('maskierery');
            ximask={};
            yimask={};
...
if size(resultslist,2)>=frame && numel(resultslist{1,frame})>0 %analysis exists
    ...
    if deriv==10
        ...
        %so gross machen wie aktuelles fenster...
...
x=resultslist{1,frame};
y=resultslist{2,frame};
text(mean(x(1,:)/1.5),mean(y(:,1)), 'Please wait. LIC in progress...','tag', 'waitplease', 'backgroundcolor', 'k', 'color', 'r','fontsize',12);
drawnow;
iterations=2;
...
if isequal(FileName,0) | isequal(PathName,0)
    ...
    hgui=getappdata(0,'hgui');
    warning off all
    vars=load(fullfile(PathName,FileName),'yposition', 'FileName', 'PathName', 'add_header', 'addfileinfo', 'autoscale_vec', 'caliimg', 'caluv', 'calxy', 'cancel', 'clahe_enable', 'clahe_size', 'clip_thresh', 'colormap_choice', 'delimiter', 'derived', 'displaywhat', 'distance', 'enable_clip', 'enable_highpass', 'enable_intenscap', 'epsilon', 'filename', 'filepath', 'highp_size', 'homedir', 'img_not_mask', 'intarea', 'interpol_missing', 'loc_med_thresh', 'loc_median', 'manualdeletion', 'maskiererx', 'maskierery', 'pathname', 'pointscali', 'resultslist', 'roirect', 'sequencer', 'sessionpath', 'stdev_check', 'stdev_thresh', 'stepsize', 'subpix', 'subtr_u', 'subtr_v', 'toggler', 'vectorscale', 'velrect', 'wasdisabled', 'xposition','realdist_string','time_inp_string','streamlinesX','streamlinesY','manmarkersX','manmarkersY','imginterpol','dccmark','fftmark','pass2','pass3','pass4','pass2val','pass3val','pass4val','step2','step3','step4','holdstream','streamlamount','streamlcolor','ismean');
...
if formattype==1
    ...
    aviobj = close(aviobj);
elseif formattype ==2 || formattype==3 || formattype==4
    if formattype==2
        ...
        if selectionok==1
            ...
            if sizeerror==0
                ...
                if selectionok==1
                    ...
                    filename{size(filename,1)+1,1}=['MEAN, frames ' str];
                    filename{size(filename,1)+1,1}=['MEAN, frames ' str];
